...
no_a=float(na[0]["price"]);no_b=float(nbids[0]["price"])if nbids else no_a*0.98
   ...
   cheap="YES"if yes_m<=no_m else"NO";cheap_p=min(yes_m,no_m);diff=abs(no_m-yes_m)
   ts=None;tst=0
 ...
 return res
app=Flask(__name__)
bot_pk=os.environ.get("POLY_PRIVATE_KEY","");bot_maker=""
bot_pnl=0.0;bot_trades=0;bot_wins=0;bot_losses=0;bot_cycle=0
bot_positions=[];bot_history=[];last_scan="尚未扫描";last_trade="暂无";last_scan_data={}
...
def trade_loop():
 ...
 while True:
  try:
   bot_cycle+=1;now=datetime.now();minute=now.minute%5;sec=now.second
   log.info(f"\n{bot_cycle}轮 {now.strftime('%H:%M:%S')} {minute}m{sec}s 开仓{len(bot_positions)}笔 PnL:${bot_pnl:.2f}")
   for p in bot_positions[:]:
    hid=(time.time()-p["ot"])/60;tid=p["tid"];bp=p["bp"];sz=p["sz"];side=p["side"]
    book=_get(f"{CLOB}/book?token_id={tid}")
    if not book:continue
    bids=book.get("bids",[]);cur=float(bids[0]["price"])if bids else None
    if not cur:continue
    ...
    if sell:
     ...
     rem=sz-ss
     if rem<0.01:bot_positions.remove(p);log.info(f"平 {p['name']} PnL:${n:+.4f}")
     else:p["sz"]=rem;log.info(f"部分平 {p['name']} 剩{rem:.2f}")
   markets=scan();last_scan_data=markets
   opps=[]
   for slug,m in markets.items():
    ...
    if diff>=0.02 or m["is_extreme"]:
     ...
     opps.append((slug,name,m["condition_id"],m["token_yes"]if cheap=="YES"else m["token_no"],cheap,cheap_p,sz,m["is_extreme"]))
    ...
    if ts and diff<0.10 and tst>0.01:
     ...
     opps.append((slug,name,m["condition_id"],m["token_yes"]if ts=="YES"else m["token_no"],ts,tp,BET*0.3,False))
     log.info(f"{name}:追{ts}@{tp:.4f}")
   for slug,name,cid,tid,side,price,sz,extreme in opps:
   ...
   log.info(f"PnL:${bot_pnl:.2f} 开仓{len(bot_positions)}笔 历史{bot_trades}笔")
...
def home():
 ...
 for s in ["btc-5m","eth-5m","sol-5m","xrp-5m","doge-5m","hype-5m","bnb-5m"]:
  ...
  if m:mr+=f"<tr><td>{m['name']}</td><td>{m['yes_mid']:.4f}</td><td>{m['no_mid']:.4f}</td><td>{m['cheap_side']}@{m['cheap_price']:.4f}</td><td>{m['price_diff']:.4f}</td><td>u{m['yes_trend']}/d{m['no_trend']}</td></tr>"
  else:mr+=f"<tr><td>{s[:3].upper()}</td><td colspan=5 style='color:#666'>无数据</td></tr>"
 ph=""
 for p in bot_positions:
  h=(time.time()-p["ot"])/60
  ph+=f"<div>{p['name']} {p['side']} @${p['bp']:.4f}x{p['sz']:.1f} hold{h:.0f}m</div>"
 rh=bot_history[-5:]if bot_history else[]
 hh="<br>".join(f"{'G'if h['pnl']>=0 else 'R'}{h['name']}${h['pnl']:+.2f}"for h in reversed(rh))if rh else"<div>暂无</div>"
...
<h1>PM Bot</h1>
<div class=card><div class=pnl>${bot_pnl:.2f}</div><div>{bot_trades}笔 {bot_wins}W/{bot_losses}L 胜率{round(bot_wins/bot_trades*100,1)if bot_trades else 0}% | {bot_cycle}轮 | 开仓{len(bot_positions)}</div></div>
<div class=card><h2>7币种</h2><table><tr><th>币</th><th>Yes</th><th>No</th><th>便宜边</th><th>价差</th><th>趋势</th></tr>{mr}</table></div>
<div class=card><h2>开仓</h2>{ph if ph else '<div class=gray>无</div>'}</div>
<div class=card><h2>最近</h2>{hh}</div>
</body></html>"""
@app.route("/api/status")
def st():return jsonify({"pnl":round(bot_pnl,2),"trades":bot_trades,"wins":bot_wins,"losses":bot_losses,"cycle":bot_cycle,"positions":len(bot_positions)})
@app.route("/api/history")
def h():return jsonify(bot_history[-50:])
if __name__=="__main__":
 if not bot_pk:log.error("无密钥");exit(1)
 log.info(f"钱包:{bot_maker[:10]}..|$4/边|上限$20")
